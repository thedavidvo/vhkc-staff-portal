    ...
    <>
      ...
      >
        {loading ? (
          ...
          <>
            <SectionCard title="Seasons" icon={Calendar} className="mb-6">
                ...
                <div className="flex gap-3 min-w-max pb-1">
                  {sortedSeasons.map((season) => (
                    <button
                      ...
                      className={`min-w-[240px] text-left bg-white dark:bg-slate-900 rounded-lg border p-3 transition-colors ${
                        selectedSeason?.id === season.id
                          ? 'border-slate-900 dark:border-slate-100 ring-1 ring-slate-900 dark:ring-slate-100'
                          : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900/70'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <h2 className="text-sm font-semibold text-slate-900 dark:text-white leading-tight line-clamp-2">
                          {season.name}
                        </h2>
                      ...
                      <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-800 text-[11px] text-slate-600 dark:text-slate-400">
                        ...
                        {selectedSeason?.id === season.id && (
                          <span className="ml-2 text-slate-900 dark:text-slate-100 font-semibold">Selected</span>
                        )}
                      </div>
                    </button>
                  ))}
                ...
                {displayedRounds.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {displayedRounds.map((round) => (
                      <div
                        key={round.id}
                        className="p-3 bg-white dark:bg-slate-900 rounded-md border border-slate-200 dark:border-slate-800 relative group cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900/70 transition-colors"
